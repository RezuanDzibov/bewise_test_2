...
async def test_with_invalid_user_id(auth_test_client_and_user: list, wav_file_in_bytes: bytes):
...
async def test_file_with_invalid_extension(auth_test_client_and_user: list, wav_file_in_bytes: bytes):
    auth_test_client, user = auth_test_client_and_user
    response = await auth_test_client.post(
        "/audiotrack",
        data={"user_id": user.id},

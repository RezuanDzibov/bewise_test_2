...
async def test_with_valid_file_and_data(
    ...
    audiotrack_id = await insert_audiotrack_and_get_it_id(session=session, file=upload_file, user_id=user.id)
    ...
    assert await get_filename_with_mp3_extension(upload_file.filename) == audiotrack_in_db.filename

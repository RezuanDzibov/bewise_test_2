...
from services.audiotracks import (
    insert_audiotrack_and_get_it_id,
    get_filename_with_mp3_extension,
)
...
async def test_with_valid_file_and_data(
    ...
    audiotrack_id = await insert_audiotrack_and_get_it_id(
        session=session, file=wav_upload_file, user_id=user.id
    )
    ...
    assert (
        await get_filename_with_mp3_extension(wav_upload_file.filename)
        == audiotrack_in_db.filename
    )
...
async def test_with_corrupt_file(
    ...
    with pytest.raises(AudioFileCorruptException):
        await insert_audiotrack_and_get_it_id(
            session=session, file=invalid_upload_file, user_id=user.id
        )
...
async def test_file_with_mp3_extension(
    ...
    with pytest.raises(AudioFileCorruptException):
        await insert_audiotrack_and_get_it_id(
            session=session, file=mp3_upload_file, user_id=user.id
        )

...
async def insert_user(session: AsyncSession, user_in_schema: UserInSchema) -> UserOutSchema:
    statement = insert(User).values(user_in_schema.dict())
    statement = statement.returning(User)
    result = await session.execute(statement)
    user_in_db = result.scalar()
    user_out_schema = UserOutSchema(id=user_in_db.id, access_token=user_in_db.access_token)
